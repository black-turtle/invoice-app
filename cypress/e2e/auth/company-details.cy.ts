...
describe('/company-details access', () => {
    CyCantAccessPageWithoutLogin('http://localhost:3000/company-details')
})
...
describe('/company-details', () => {
    ...
    it('can fill in values the first time', () => {
        ...
        cy.get('[data-test="company-address"]').type(
            `${ONBOARDING_COMPANY_ADDRESS}`,
        )
        cy.get('[data-test="company-vat"]').type(`${ONBOARDING_COMPANY_VAT}`)
        cy.get('[data-test="company-reg-number"]').type(
            `${ONBOARDING_COMPANY_REG}`,
        )
        cy.get('[data-test="company-iban"]').type(`${ONBOARDING_COMPANY_IBAN}`)
        cy.get('[data-test="company-swift"]').type(
            `${ONBOARDING_COMPANY_SWIFT}`,
        )
        cy.get('[data-test="submit-company-details"]').click()
    ...
    it('can update company details after initial submission', () => {
        ...
        cy.get('[data-test="company-name"]').should(
            'have.value',
            `${ONBOARDING_COMPANY_NAME}`,
        )
        cy.get('[data-test="company-address"]').should(
            'have.value',
            `${ONBOARDING_COMPANY_ADDRESS}`,
        )
        cy.get('[data-test="company-vat"]').should(
            'have.value',
            `${ONBOARDING_COMPANY_VAT}`,
        )
        cy.get('[data-test="company-reg-number"]').should(
            'have.value',
            `${ONBOARDING_COMPANY_REG}`,
        )
        cy.get('[data-test="company-iban"]').should(
            'have.value',
            `${ONBOARDING_COMPANY_IBAN}`,
        )
        cy.get('[data-test="company-swift"]').should(
            'have.value',
            `${ONBOARDING_COMPANY_SWIFT}`,
        )
        ...
        cy.get('[data-test="company-name"]')
            .clear()
            .type(`new ${ONBOARDING_COMPANY_NAME}`)
        cy.get('[data-test="company-address"]')
            .clear()
            .type(`updated ${ONBOARDING_COMPANY_ADDRESS}`)
        cy.get('[data-test="company-vat"]')
            .clear()
            .type(`updated ${ONBOARDING_COMPANY_VAT}`)
        cy.get('[data-test="company-reg-number"]')
            .clear()
            .type(`updated ${ONBOARDING_COMPANY_REG}`)
        cy.get('[data-test="company-iban"]')
            .clear()
            .type(`updated ${ONBOARDING_COMPANY_IBAN}`)
        cy.get('[data-test="company-swift"]')
            .clear()
            .type(`updated ${ONBOARDING_COMPANY_SWIFT}`)
        cy.get('[data-test="submit-company-details"]').click()
